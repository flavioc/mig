...
WriteExtractArgValue(FILE *file, const argument_t *arg)
{
	...
	if (have_payload) {
		...
		WriteCopyType(file, it, "%s", "/* %s */ %s(%s%s)",
			      ...
			      InArgMsgField(arg), is_inlined_port ? ".name" : "");
	} else {
	    WriteCopyType(file, it, "%s", "/* %s */ %s(%s%s)",
			  arg->argVarName, it->itInTrans,
			  InArgMsgField(arg),
			  is_inlined_port ? ".name" : "");
	}
    } else {
	WriteCopyType(file, it, "%s", "/* %s */ %s%s",
		      arg->argVarName, InArgMsgField(arg), is_inlined_port ? ".name" : "", arg->argVarName);
    }
    fprintf(file, "\n");
}
...
WriteServerCallArg(FILE *file, const argument_t *arg)
{
    ...
    const bool inlined_port_name = it->itUserlandPort &&
	...
	fprintf(file, "%s", arg->argVarName/*, inlined_port_name ? ".name" : ""*/);
	...
	if (akCheck(arg->argKind, akbIndefinite)) {
	    ...
	    fprintf(file, "%s%s", InArgMsgField(arg), inlined_port_name ? ".name" : "");
    }
    else
	fprintf(file, "OutP->%s%s", arg->argMsgField, inlined_port_name ? ".name" : "");
...
WritePackArgValue(FILE *file, const argument_t *arg)
{
	...
	WriteCopyType(file, it, "OutP->%s%s", "/* %s%s */ %s(%s)",
		      arg->argMsgField, is_inlined_port ? ".name" : "", it->itOutTrans, arg->argVarName);
    } else
	WriteCopyType(file, it, "OutP->%s%s", "/* %s%s */ %s",
		      arg->argMsgField, is_inlined_port ? ".name" : "", arg->argVarName);
}
